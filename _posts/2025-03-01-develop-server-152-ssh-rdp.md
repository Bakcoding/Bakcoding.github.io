...
<p>사용자가 그래픽 환경을 원격으로 사용할 수 있도록 즉, 데스크톱 화면을 그대로 보면서 마우스와 키보드로 제어할 수 있는 방식으로 <span style="text-align: start">일반적으로 윈도우 환경에서 많이 사용된다.</span><span style="letter-spacing: 0px"> GUI를 그대로 볼 수 있기 때문에 사용이 편리하고 직관적이라 원격으로 그래픽 환경이 필요한 작업을 할 때 유용하다.</span></p>

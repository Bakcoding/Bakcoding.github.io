...
* unordered_set 활용
  ...
  해시를 사용하니 더 빠른 속도로 결과를 냈다. set과 map은 몇 가지 종류가 존재하는데 unodered_set을 사용한 이유는 정렬되지 않은 데이터에(unordered)를 키와 연관된 데이터의 존재 유무만 파악(set)하면 되기 때문이다. 그리고 동명이인의 중복된 데이터 처리를 위해서 multiset을 사용한 것이다.  

  동일한 해시 방식이라도 필요한 기능부분만 사용하는 것이 더 빠른 속도를 낸다.  

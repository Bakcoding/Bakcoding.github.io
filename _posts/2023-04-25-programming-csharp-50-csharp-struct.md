...
<p>패딩은 구조체의 크기를 불필요하게 늘리기도 하므로 크기를 최소화하기 위해서는<span style="text-align: start"><span>&nbsp;</span>큰 데이터를 먼저 선언하고 뒤에 작은 데이터들이 선언되도록</span> 멤버들을 크기에 따라 정렬하는것이 좋다. 이렇게 구조체를 제대로 이해하고 사용하기 위해서는 각 타입들의 크기와 패딩의 존재에 대해 정확한 이해가 필요하다.</p>
